...
const Location = (p) => {
	...
	useEffect(() => {
		...
		document.getElementById('btnPayment').onclick = async () => {
			...
			if (status === 200) p.navigation.navigate('Payment', { url: data })
		}
	...
	return (
		<div style={{ width: '100%', height: '100%', minWidth: '100%', minHeight: '100%' }}>
			...
			<div id='bottomDiv' style={{ visibility: p.route.params?.origin ? 'hidden' : 'visible', zIndex: 10000, position: 'absolute', bottom: 0, width: '97%', background: '#fff', padding: '8px 10px 5px' }}>
				<div id='bottomDiv2' style={{ display: 'flex', justifyContent: 'flex-start', width: '100%', background: '#fff', padding: '8px 0 8px', flexDirection: 'row' }}>
					...
					<button id='btnPayment' style={{ display: 'block', marginRight: 12, border: '1px solid #3af', background: "#3afa", height: '42px', width: '75px', fontSize: '15px', borderRadius: '5px', color: '#444', }} >پرداخت</button>
				</div>
