...
export const Layout = (p) => {
  ...
  let bottom = (p.tokenValue.isAdmin !== 'courier') ?
    ...
    { title: (p._key == '100') ?
     p.route.name : 'Profile', icon: 'user', navigate:p.route.name === 'FinallFoodPayment' || p.route.name ===  'Profile' || p.route.name ===  'Logout'?p.route.name: p.navigateProfile }
     :
      { title: (p._key == '120') ?
  ...
  return (
    p.route.name === 'Profile' || p.route.name === 'FinallFoodPayment' | p.route.name === 'Login' | p.route.name === 'Register' | p.route.name === 'Home' ?
      <Animated.View style={[
        (Platform.OS === 'ios') ?
          p.width < p.height ?
