...
const DeleteAllAddress = (p) => {
	...
	return (
		<View style={{ flex: 1, width: '100%', backgroundColor: '#eee' }} >
			...
			{p.showBtn && <Button alignSelf='center' outline w='90%' bgcolor='silver' onPress={() => { p.setshowBtn(!p.showBtn) }} >جمع ماه گذشته از {p.oldPrice?.split("=") && p.oldPrice.split("=")[0]} = {p.oldPrice?.split("=") && p.spacePrice(p.oldPrice.split("=")[1])} تومان</Button>}
