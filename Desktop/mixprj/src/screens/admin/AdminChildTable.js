...
const AdminChildTable = (p) => {
  ...
  return (
    <Span flex={1} class={s.container}>

      <Span class={s.containChildTable} >
        <Button h={32} ph={8} mv={5} onPress={() => p.navigation.navigate("CreateChildFood", { id: p.route.params.id, title:p.route.params.title })}>ساخت </Button>
        <Span class={s.containerChildTable}>
          {!p.current ?
            <Loading />
            :
            <Table
