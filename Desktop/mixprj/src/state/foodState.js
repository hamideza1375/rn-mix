...
export const home = (p) => {
  ...
  p.useEffect(() => {
    ...
    Axios.interceptors.response.use(function (response) {
      if(response.config.method !== 'get' && location.pathname !== '/Payment' && location.pathname !== '/Location' && (response.status === 200 || response.status === 201)) toastOK()
      return response  
    }, function (error) {
  ...
  p.useEffect(()=>{
    (async()=>{
      const token = await localStorage.getItem("token"); if (token) Axios.defaults.headers.common["Authorization"] = token
    })()
  },[p.changeLoginUser])
