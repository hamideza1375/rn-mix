...
<Text style={[{ textAlign: 'center', paddingVertical: 10, }, props.textStyle]}> {props.children}</Text></View>
...
<Text style={[{ textAlign: 'center', color: '#222', paddingVertical: 10, }, props.textStyle]}> {props.children}</Text></View>

const Tbtn = (props) => <View style={[{ flex: 1, backgroundColor: 'white', borderColor: '#666', borderWidth: .8, justifyContent: 'center', alignItems: 'center', borderRadius: 1.5, }, props.style]}><Button {...props} textStyle={[{ fontSize: 15 }, props.textStyle]} style={[{ width: '99.9%', flex: 1 }, { paddingHorizontal: 0 }]}>{props.children}</Button></View>
let odd = [];
  ...
  if (!object)
    ...
    return (
      <View style={{ width: w, maxHeight: '100%', marginTop: mt, alignItems: 'flex-start', }}>
        <Row flexDirection='row-reverse' w='100%' alignSelf='center'>
