...
import { StyleSheet, FlatList, ImgBackground, Input, P, Press, Span, Icon, Animated, Img, Container } from '../../Components/Html'
 ...
 setInterval(() => {
  ...
  return (
    <Container >
      ...
      <Span f={Platform.OS === 'web'?1:.8} class={s.viewContainer} style={p.width > p.height && { paddingHorizontal: 22, alignSelf: 'center' }} >
        <>
          {!p.current ?
            <Loading />
            :
                ...
                <>
                  <Span style={[styleScroll, { opacity: item.available ? 1 : .4 }, { height: 180 },
                    ...
                    <Press style={{ height: '55%', backgroundColor: '#eee' }} onPress={() => { p.navigation.navigate("SingleFood", { title: item.title, id: p.route.params.id, id2: item._id, page: p.page }) }} >
                      <ImgBackground class={[s.img, s.radius]} containClass={s.radius} source={{ uri: `${localhost}/upload/food/${item.imageUrl}` }}>
        ...
        {item2.title && item2.title != item3.title && <Animated.View style={[{ height: 200, width: 200, transform: [{ scale: p.animScale }], position: 'absolute', borderRadius: 11, borderWidth: 15, borderColor: 'silver', zIndex: 100, top: hidden, left: hidden2 }]} >
          <ImgBackground class={s.img} source={{ uri: `${localhost}/upload/food/${item2.imageUrl}` }}>
        ...
        f={.2}
          ...
          <Img class={s.imagePayment} source={{uri:`${localhost}/images/food_image.jpg`}} />
