...
import { Icon, Button, ImgBackground, Span, P , FlatList, StyleSheet} from '../../Components/Html'
    ...
    <Span class={s.s_container} >
        ...
        <Span class={s.imgBackgroundContainer} >
          {
            p.singlefood && p.singlefood.title ?
              <>
                <Span class={s.head}>
                  <ImgBackground style={{height: '100%'}} source={{ uri: `${p.localhost}/upload/food/${p.singlefood.imageUrl}` }} >
      ...
      <Span w='100%'>
        ...
        <Span class={s.btnContainer} >
          {!p.showForm && <Button mt={7} w={'85%'} bgcolor='#f82' color="#333" class={s.btnShow} onClick={() => { p.set ? p.setshowForm2(!p.showForm2) : editComment(p.allcomment.find(comment => comment.starId === p.tokenValue.userId)?._id) }}>
              {!p.showForm2 ? p.sendMessage ? ' ارسال نظر' : ' ویرایش نظر' : ' بازگشت'}
            </Button>}
          {!p.showForm2 && p.showForm && <Button mt={7} w={'85%'} bgcolor='#f82' color="#333" class={s.btnShow} onClick={() => { p.setshowForm(false) }}>
            بازگشت
          </Button>}
     ...
     <Span w={Platform.OS === 'web' ?'70%':'100%'} as={'center'} h='60%' border={[1, '#f80']} >
        {
          // p.showForm && p.showForm2 &&
        ...
        <Span class={s.paginationContainer} >
          {p.allcomment.length ?
           <Pagination
            food={[...p.allcomment, ...p.allcomment, ...p.allcomment , ...p.allcomment,...p.allcomment, ...p.allcomment, ...p.allcomment , ...p.allcomment]}
