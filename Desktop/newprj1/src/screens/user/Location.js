...
const Location = (p) => {
  ...
  GetLocation.getCurrentPosition({ enableHighAccuracy: false, timeout: 15000 })
    .then(location => { console.log({ lat: location.latitude, lng: location.longitude, }); p.setregion({ lat: location.latitude, lng: location.longitude, }) })
...
<div style="display: flex; justify-content: flex-end; height: 100%; ">
  ...
  <form id="formSearch" onsubmit="submited(event)"
    ...
    <i onclick="sendIcon()" class="fa fa-search"
      ...
      <button id='btnGetLocation' onclick="setLocation()" style='display: flex; alignItems: center; justifyContent: center; textAlign: center; background: #fff;padding: 0; borderRadius: 4px; zIndex: 10000; top: 74; left: 9.88; fontSize: 20px; height: 31px; maxHeight: 31px; width: 32.8px; maxWidth: 32.8px; borderWidth: 0; boxShadow: .2px 1.5px 4px #333d;z-index:10000;position:absolute;border-radius:4px;align-Items:center;justify-content:center'><p style="transform: rotate(-65deg); padding: 0; margin: 0; font-size:24px; text-align:center; margin:auto; margin-top:-5px " >⌲</p></button>
      <button id='btnGetLocation2' onclick="setLocation2()" style='display: flex; alignItems: center; justifyContent: center; textAlign: center; background: #fff;padding: 0; borderRadius: 4px; zIndex: 10000; top: 74; left: 9.88; fontSize: 20px; height: 31px; maxHeight: 31px; width: 32.8px; maxWidth: 32.8px; borderWidth: 0; boxShadow: .2px 1.5px 4px #333d;z-index:10000;position:absolute;border-radius:4px;align-Items:center;justify-content:center'><p style="transform: rotate(-65deg); padding: 0; margin: 0; font-size:24px; text-align:center; margin:auto; margin-top:-5px " >⌲</p></button>
      ...
      document.getElementById('bottomDiv').style.display = origin && 'none'
        ...
        document.getElementById('bottomDiv').style.bottom = -90
        })

        marker.on('dragend', async (ev) => {
          document.getElementById('bottomDiv').style.bottom = 0
