...
import { H1, Div, Input, Row } from "./Components/Html";
import { Init } from "./Components/Html";
import { Layout, header } from "./_other/Layout";
  ...
  return (
    ...
    :
      ...
      {/* <contextStates.Provider value={p}> */}
        ...
        <ToastProvider {...p} />
        <Tab.Navigator screenOptions={(props) => ({ headerLeft: () => <Icon style={{ fontSize: 28, paddingRight: 15, color: '#777' }} onPress={() => props.navigation.navigate('Home')} />, headerTitleAlign: 'center', headerTitleStyle: { color: 'transparent' } })} >
          <Tab.Group >
            <Tab.Screen name="Home" options={(props)=>({ title: 'خانه', headerStyle: { backgroundColor: '#103' }, headerLeft: p.tokenValue.fullname !== '' ? () => <></> : () => <Pressable style={{ width: 80, height: 40, justifyContent: 'center', alignItems: 'center', marginRight: 10, backgroundColor: '#444', borderRadius: 4, shadowRadius: 9, shadowOpacity: .4, shadowColor: 'blue' }} onPress={() => props.navigation.navigate('Register')} ><Text style={{ color: '#fff', fontSize: 16 }}>ورود/ثبت نام</Text></Pressable> })} children={(props) => <Layout {...props}  {...p} ><Home {...props} {...p}  {...reducer(props)} /></Layout>} />
